...
export default function BottomNav() {
  return (
    <nav className="w-full z-40 bg-gray-100 border-t border-gray-200">
...
function NavItem({ to, icon: Icon, label }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `flex flex-col items-center justify-center gap-1.5 px-3 py-2 rounded-2xl text-xs sm:text-sm transition-all duration-200 ${isActive
    ...
    >
      {({ isActive }) => (
        <>
          <Icon
            className={`w-5 h-5 sm:w-6 sm:h-6 transition-transform duration-200 ${isActive ? "scale-110" : ""
              }`}
